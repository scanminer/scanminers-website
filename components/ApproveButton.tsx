...
export function ApproveButton({ path, slug }: Props) {
    ...
    try {
      ...
      const res = await fetch("/api/admin/approve", {
        ...
        headers: {
          ...
          "x-admin-action-token": token,
        },
        body: JSON.stringify({ path, branch, title: slug ? `Approve content: ${slug}` : undefined }),
      });
      if (res.ok) {
        const data = await res.json().catch(() => ({}));
        setMsg("PR opened");
        toast("Approve PR opened", "success");
        if (data.pr_url) setPrUrl(data.pr_url);
        setLastBranch(branch);
        setOpen(false);
      } else {
        const text = await res.text();
        toast("Approve failed: " + text, "error");
        setMsg("Failed: " + text);
      }
    } catch (err) {
  ...
  return (
    <span className="inline-flex items-center gap-2">
      ...
      >
        {busy ? "Approving…" : !path ? "No source" : "Mark Approved"}
      </button>
      {msg ? (
        prUrl ? (
          <a
