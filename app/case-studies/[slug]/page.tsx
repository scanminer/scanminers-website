...
export default async function CaseStudyPage({ params }: PageProps) {
  ...
  const study = allCaseStudies.find((s: CaseStudy) => s.slug === slug) as
    | CaseStudy
    | undefined;
  ...
  else if (Array.isArray(doc.commodity))
    commodities = doc.commodity as string[];
        ...
        <article>
          ...
          <section className="mt-16 rounded-2xl border-2 border-primary/30 bg-gradient-to-br from-primary/10 via-primary/5 to-transparent p-8 text-center shadow-xl">
            ...
            <p className="text-muted-foreground mb-6 leading-relaxed">
              If you&apos;re exploring for{" "}
              {commodities.length > 0
                ? commodities.join(", ").toLowerCase()
                : "critical minerals"}{" "}
              or other minerals in complex or covered terrain, Scanminers can
              run a similar remote sensing and AI study for your region — from
              multi-sensor fusion maps to ranked targets and decision-ready
              reports.
            </p>
