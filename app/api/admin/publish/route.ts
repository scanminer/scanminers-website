...
export async function POST(req: Request) {
  try {
    ...
    const body = await req.json().catch(() => ({})) as { path?: string; branch?: string; title?: string; merge_method?: 'merge' | 'squash' | 'rebase' };
    ...
    if (updated === original) {
      // Nothing to change; proceed to create an empty PR? Instead, return early.
      return new Response('No change', { status: 400 });
    }
    ...
    const branchName = `publish/${slugPart}-${Date.now()}`;
    ...
    await gh(`/repos/${owner}/${repo}/contents/${encodeURIComponent(path)}`, {
      ...
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        message: `chore(admin): publish content ${slugPart}`,
    ...
    // Open PR
    const prTitle = body.title || `Publish content: ${slugPart}`;
    const prResp = await gh(`/repos/${owner}/${repo}/pulls`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ title: prTitle, head: branchName, base: baseBranch, body: 'Publish via Admin UI' }),
    });
    ...
    try {
      ...
      const mr = await mergeResp.json();
      merged = Boolean(mr.merged);
    } catch {
      // merging may fail due to checks/branch protections; fall through
    }

    return json(202, { pr_url: pr.html_url, merged });
  } catch (e: unknown) {
