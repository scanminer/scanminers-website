...
grep -R --line-number --color=never -E "next/font|next/navigation|contentlayer|@contentlayer|shadcn|/ui/|MDXProvider|next-mdx|plausible|Plausible" \
  -n app components content lib scripts workers \
  --exclude-dir=node_modules --exclude-dir=.next --exclude-dir=.git --exclude-dir=.contentlayer --exclude-dir=public --exclude-dir=.cursor || true

say "CHECK: Components we discussed"
grep -R --line-number --color=never -E "TwoLaneProcess|CompareSlider|ReviewQueue|validate-citations|Gate [ABCD]|review_status|ai_generated" \
  -n app components content lib scripts workers \
  --exclude-dir=node_modules --exclude-dir=.next --exclude-dir=.git --exclude-dir=.contentlayer --exclude-dir=public --exclude-dir=.cursor || true
...
grep -R --line-number --color=never -E "Cloudflare|wrangler|Turnstile|@cloudflare|window\._paq|gtag|plausible" \
  -n app components content lib scripts workers \
  --exclude-dir=node_modules --exclude-dir=.next --exclude-dir=.git --exclude-dir=.contentlayer --exclude-dir=public --exclude-dir=.cursor || true
