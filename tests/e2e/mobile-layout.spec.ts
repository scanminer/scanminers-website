...
test.describe("Mobile layout", () => {
  ...
  test("mobile menu opens and aligns", async ({ page }) => {
    ...
    await expect(
      panel.getByRole("link", { name: "About", exact: true })
    ).toBeVisible();
    ...
    if (btnBox) {
      // Allow up to 32px from right edge for various padding configurations
      expect(viewport.width - (btnBox.x + btnBox.width)).toBeLessThanOrEqual(
        32
      );
    }
    ...
    await expect(
      panel.getByRole("link", { name: "Contact" }).first()
    ).toBeVisible();
  });
});
